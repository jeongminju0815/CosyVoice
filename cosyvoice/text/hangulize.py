...
class EnglishToHangul(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.consonants = {
            ...
            'm':'ㅁ', 'n':'ㄴ', 'ɲ':'N', 'ŋ':'ㅇ', 'l':'ㄹ', 'r':'ㄹ', 'h':'ㅎ', 'ç':'ㅎ', 'x':'ㅎ', 'ɹ': 'ㄹ', 'ɡ': 'ㄱ', 'ɾ':'ㅌ',
            # semivowels
            'j':'ㅣ', 'ɥ':'ㅟ', 'w':'ㅜ'
        }
        self.vowels = {
            ...
            'a':'ㅏ', 'ɑ':'ㅏ', 'ɑ̃':'A', 'ʌ':'ㅓ', 'ɔ':'ㅗ', 'ɔ̃':'O', 'o':'ㅗ', 'ə':'ㅓ', 'ɚ':'ㅓ', 'ɐ': 'ㅓ',
            'ʊ':'ㅜ', 'u':'ㅜ', 'ᵻ': 'ㅣ', 'ɜ': 'ㅓ'
        }
    ...
    def syllabify(self) -> None:
        ...
        self.phonetic = self.phonetic.replace('tʃ', 'ʧ')
        # self.phonetic = self.phonetic.replace('oʊ', 'o')
        ...
        self.phonetic_syllabic = re.sub('(?<=[iɪyeøɛœæaɑʌɔoəɚuɐᵻɜ])([pbtdkgfvθðszʃʒʦʣʧʤɲŋlrhçx])l', '/\\1l', self.phonetic_syllabic) 
        self.phonetic_syllabic = re.sub('(?<=[iɪyeøɛœæaɑʌɔoəɚuɐᵻɜ])/l', 'l/l', self.phonetic_syllabic) 
        ...
        self.phonetic_syllabic = re.sub('d/ʒ', '/ʒ', self.phonetic_syllabic)
        ...
        self.phonetic_syllabic = re.sub('(?<=o)/ʊ', '/u', self.phonetic_syllabic)
    ...
    def transcribe_english(self, word) -> str:
        ...
        if self.errata.get(word):
            ...
            if filter_roman_numerals(word):
                self.phonetic = eng_to_ipa.convert(word)
            else:
                self.phonetic = phonemize(word, backend='espeak', language='en-us', strip=True)
...
if __name__ == '__main__':
    ...
    for phrase in args.words:
        ...
        output.clear()
        for word in words:
            if p.match(word[0]) and not word.isupper():
